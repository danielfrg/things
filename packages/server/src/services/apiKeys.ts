...
export type ApiKeyInfo = Pick<ApiKey, "id" | "name" | "keyPrefix" | "scope" | "lastUsedAt" | "createdAt">
...
export async function listApiKeys(userId: string): Promise<ApiKeyInfo[]> {
  const keys = (await db.select().from(apiKeys).where(eq(apiKeys.userId, userId))) as ApiKey[]
  return keys.map((key) => ({
    id: key.id,
    name: key.name,
...
export async function removeApiKey(keyId: string, userId: string): Promise<{ success: boolean }> {
  await db.delete(apiKeys).where(and(eq(apiKeys.id, keyId), eq(apiKeys.userId, userId)))
  return { success: true }
}

export async function removeApiKeyByName(name: string, userId: string): Promise<{ success: boolean }> {
  await db.delete(apiKeys).where(and(eq(apiKeys.name, name), eq(apiKeys.userId, userId)))
  return { success: true }
}

export async function revokeCurrentKey(keyHash: string): Promise<{ success: boolean }> {
