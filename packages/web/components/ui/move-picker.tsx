...
interface MovePickerContentProps {
  value?: string | null;
...
/**
 * Move picker content - for use inside a Popover.
 * Does NOT include Popover wrapper - use MovePicker for the full component with trigger.
 */
export function MovePickerContent({
  value,
  areaValue,
  onChange,
  projects,
  ...
  return (
    <div className="w-[260px] max-md:w-[calc(100vw-32px)] rounded-xl bg-popover-dark border border-popover-dark-border overflow-hidden">
      {/* Header with title */}
    ...
    if (selectedProject) {
      return (
        <ProjectProgressIcon
          progress={0}
          size={14}
          className="text-popover-dark-selected opacity-70"
        />
      );
    }
    if (selectedArea) {
