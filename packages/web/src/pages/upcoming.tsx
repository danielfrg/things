...
import { useUpcomingView } from "@/context/view-adapters"
import { useBatchOperations } from "@/lib/hooks/useBatchOperations"

export function Upcoming() {
  const app = useApp()
  const data = useUpcomingView()
  ...
  const initialTemplateId = () => {
    ...
    return typeof templateParam === "string" ? templateParam : null
  }
  ...
  const handleMove = async (info: TaskMoveInfo) => {
    ...
    const updates: Record<string, unknown> = {
      contextType: "upcoming", // Tell the server we're in Upcoming view
    }

    // Upcoming view moves tasks between date sections
    if (toSection.dateStr) {
  ...
  const handleReorder = (sectionId: string, taskIds: string[]) => {
    data.reorderTasks(taskIds, sectionId, { type: "upcoming" })
  }
  ...
  const handleTemplateUpdate = (id: string, updates: Partial<TemplateInfo>) => {
  ...
  // Check if there's any content (tasks or templates)
  const hasContent = () => data.sections.some((s) => s.tasks.length > 0 || (s.templates ?? []).length > 0)

  return (
    <ViewContainer
      title="Upcoming"
      icon={<CalendarIcon class="w-6 h-6 text-things-pink" />}
      toolbar={
        <ViewToolbar>
    ...
    >
      ...
      <Show when={!data.loading && !data.error}>
        <Show
          when={data.sections.length > 0 && hasContent()}
          fallback={
            <div class="flex flex-col items-center justify-center py-16 text-muted-foreground">
              <CalendarIcon class="w-12 h-12 mb-4 opacity-30" />
              <p class="text-sm">No upcoming tasks scheduled</p>
              <p class="text-xs mt-1">Schedule tasks for future dates to see them here</p>
            </div>
          }
        >
          <GroupedTaskList
            ...
            initialExpandedTemplateId={initialTemplateId()}
          />
        </Show>
      </Show>
    </ViewContainer>
