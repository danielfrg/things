...
/**
 * Drop indicator with terminal circle - tailwind port of @atlaskit/pragmatic-drag-and-drop-react-drop-indicator/box
 */
export function DropIndicator(props: { edge: Edge; gap: string }) {
  return (
    <div
      ...
      class={`absolute z-10 bg-things-blue pointer-events-none before:content-[''] before:w-[--terminal-size] before:h-[--terminal-size] box-border before:absolute before:border-[length:--line-thickness] before:border-solid before:border-things-blue before:rounded-full ${orientationStyles[edgeToOrientationMap[props.edge]]} ${edgeStyles[props.edge]}`}
    />
  )
}
