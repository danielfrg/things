...
export function useHotkey(
  ...
  createEffect(() => {
    const handler = (e: KeyboardEvent) => {
      ...
      const metaMatch = options?.meta === undefined ? true : options.meta === e.metaKey
      const ctrlMatch = options?.ctrl === undefined ? true : options.ctrl === e.ctrlKey
      const shiftMatch = options?.shift === undefined ? true : options.shift === e.shiftKey
