...
function printSections(
  sections: Array<{ title: string; tasks: TaskLike[] }>,
...
async function main() {
  ...
  if (command === "add") {
    ...
    if (tokens.length === 0) {
      console.error(
        "Usage: things add <description> [date:YYYY-MM-DD] [deadline:YYYY-MM-DD] [project:name]",
      );
      process.exit(1);
    }
  ...
  if (command === "logbook") {
    ...
    const tasks = byDate.filter((item) =>
      item.listId ? ids.has(item.listId) : false,
    );
